...
typedef struct page_table
{
    ...
    unsigned int table[VMEM_1_SIZE / PAGESIZE]; // Page table for the process
...
typedef struct memblock
{
    ...
    page_table_t kernel_spage_table;            // Page table for the kernel stack
...
typedef struct pcb
{
    ...
    pcb_t p_pproc;                              // Pointer to the parent process PCB
    ...
    pcb_t *p_prev;                              // Pointer to the previous element in the ready/blocked queue
    pcb_t *p_next;                              // Pointer to the next element in the ready/blocked queue
